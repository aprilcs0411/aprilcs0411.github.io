...
<br/>
<ol>
<li>x<-matrix(vector,nrow=int,ncol=int,byrow=FALSE,dimnames=list(vector_rownames,vector_colnames))
</li>   
<li>x<-vectorA; dim(x)<-c(int,int)；dimnames(x)<-list(vector_rownames,vector_colnames); 给向量赋予维度。
</li> 
<li>x<-rbind(y,z)，x<-cbind(y,z) 注意长度不同的情况。
</li></ol>
